...
* *LIMIT* - ограничение количества выбираемых записей в запросе.
Пример. Извлечь из выборки первые пять записей о проектах. 
```sql 
SELECT * id, project_name FROM projects ORDER BY DESC LIMIT 5;
```
...
```
* *Функция  CASE* - представляет собой общее условное выражение, напоминающее операторы if/else в других языках программирования:
```sql
